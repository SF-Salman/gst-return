...
function AnnualSummary({ periods }: { periods: PeriodResult[] }) {
  ...
  // All descriptions that appear in the monthly table — same order as RECON_ROWS in backend
  const ALL_DESCS = [
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-sub/40">
              ...
              {/* One set of G1 / 3B / Diff per description */}
              {ALL_DESCS.map(desc => (
                <th
                  key={desc}
                  colSpan={3}
                  className="px-3 py-2 text-center font-semibold text-tx2 whitespace-nowrap border-l border-border"
                >
                  {desc}
                </th>
              ))}
            ...
            {periods.map((p, i) => {
              ...
              return (
                <tr
                  key={i}
                  className={`transition-colors ${
                    p.overall_status === 'Matched' ? 'hover:bg-sub/40' : 'bg-err/5 hover:bg-err/10'
                  }`}
                >
...
const TABS: { id: ReconTab; label: string; ready: boolean }[] = [
  ...
  { id: 'gstr2b-books', label: 'GSTR-2B vs Books',   ready: false },
...
const downloadG3bBooksExcel = async () => {
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Total Variance',value: '₹ ' + fmt(sortedPeriods.reduce((s,p) => s + p.total_variance, 0)),
              color: sortedPeriods.some(p => p.total_variance > 1) ? 'text-err' : 'text-ok' },
          ].map(stat => (
  ...
  <div className="mx-6 rounded-2xl border border-dashed border-border bg-surf/40 p-12 text-center">
    <Layers size={28} className="mx-auto text-tx3 mb-3" />
    <p className="text-sm font-medium text-tx2">Coming soon</p>
    <p className="text-xs text-tx3 mt-1">GSTR-2B vs Books reconciliation is under development.</p>
  </div>
)}
    ...
    <div className="bg-surf rounded-2xl border border-border shadow-soft p-5 space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {(g3bBooksFiles.length > 0 || booksFile || g3bBooksResult) && (
          <button
            onClick={() => { setG3bBooksFiles([]); setBooksFile(null); setG3bBooksResult(null); setG3bBooksError(null) }}
            className="inline-flex items-center gap-1.5 px-3 h-9 rounded-xl text-tx3
              hover:text-err hover:bg-err/5 text-xs transition-all ml-auto"
          >
            <X size={13} /> Clear
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Minor Diff',      value: g3bBooksResult.summary?.minor ?? 0,       color: 'text-warn'},
          ].map(stat => (
            <div key={stat.label} className="bg-surf rounded-xl border border-border p-3 shadow-sm">
              <div className={`text-lg font-bold tabular-nums ${stat.color}`}>{stat.value}</div>
              <div className="text-[10px] text-tx3 font-medium uppercase tracking-wide mt-0.5">{stat.label}</div>
        ...
        {/* Monthly breakdown */}
        <div className="rounded-xl border border-border bg-surf shadow-sm overflow-hidden">
          <div className="px-4 py-3 bg-sub/60 border-b border-border">
            <span className="font-semibold text-sm text-tx">Monthly Comparison</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-sub/40">
                  ...
                  <th className="px-3 py-2 text-left font-semibold text-tx2">Field</th>
                  <th className="px-3 py-2 text-right font-semibold text-tx2">Books</th>
                  <th className="px-3 py-2 text-right font-semibold text-tx2">GSTR-3B</th>
                  <th className="px-3 py-2 text-right font-semibold text-tx2">Difference</th>
                  <th className="px-3 py-2 text-center font-semibold text-tx2">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {(g3bBooksResult.monthly ?? []).map((period: any) =>
                  (period.rows ?? []).map((row: any, ri: number) => (
                    <tr key={`${period.period}-${ri}`}
                      className={row.status === 'Mismatch' ? 'bg-err/5' : row.status === 'Minor Difference' ? 'bg-warn/5' : 'hover:bg-sub/30'}>
                      {ri === 0 && (
                        <td className="px-3 py-2 font-medium text-tx whitespace-nowrap" rowSpan={period.rows.length}>
                          {period.period}
                        </td>
                      )}
                      <td className="px-3 py-2 text-tx">{row.description}</td>
                      ...
                      <td className={`px-3 py-2 text-right tabular-nums font-medium ${row.difference !== 0 ? 'text-err' : 'text-tx3'}`}>
                        {row.difference !== 0 ? (row.difference > 0 ? '+' : '') + fmt(row.difference) : '—'}
                      </td>
                      <td className="px-3 py-2 text-center">
                        {row.status === 'Match'
                          ? <span className="pill pill-lo">Match</span>
                          : row.status === 'Minor Difference'
                          ? <span className="pill pill-inf">Minor</span>
                          : <span className="pill pill-hi">Mismatch</span>}
                      </td>
                    </tr>
                  ))
